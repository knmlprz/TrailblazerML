...
class RoverController:
    ...
    def create_fuzzy_controller(self):
        ...
        left_wheel_speed["slow"] = fuzz.trapmf(
            left_wheel_speed.universe, [0, 0, 30, 50]
        )
        left_wheel_speed["medium"] = fuzz.trimf(left_wheel_speed.universe, [30, 50, 70])
        left_wheel_speed["fast"] = fuzz.trapmf(
            left_wheel_speed.universe, [50, 70, 100, 100]
        )

        right_wheel_speed["slow"] = fuzz.trapmf(
            right_wheel_speed.universe, [0, 0, 30, 50]
        )
        right_wheel_speed["medium"] = fuzz.trimf(
            right_wheel_speed.universe, [30, 50, 70]
        )
        right_wheel_speed["fast"] = fuzz.trapmf(
            right_wheel_speed.universe, [50, 70, 100, 100]
        )

        # Definicja reguł logiki rozmytej
        rule1 = ctrl.Rule(
            angle["left"], (left_wheel_speed["slow"], right_wheel_speed["fast"])
        )
        rule2 = ctrl.Rule(
            angle["straight"], (left_wheel_speed["fast"], right_wheel_speed["fast"])
        )
        rule3 = ctrl.Rule(
            angle["right"], (left_wheel_speed["fast"], right_wheel_speed["slow"])
        )
    ...
    def read_gps_data(self):
        with open(self.gps_file, "r") as file:
            ...
            self.path.append(
                (self.latitude_current, self.longitude_current)
    ...
    def calculate_distance(self):
        return math.sqrt(
            (self.latitude_goal - self.latitude_current) ** 2
            + (self.longitude_goal - self.longitude_current) ** 2
        )
    ...
    def move_rover(self, left_speed, right_speed):
        ...
        print(
            f"Moving with left wheel speed: {left_speed} and right wheel speed: {right_speed}"
        )
