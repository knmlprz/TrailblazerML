...
class CameraOAK:
    ...
    def get_data(self):
        ...
        """
        imu_queue = self.device.getOutputQueue(name="imu", maxSize=1, blocking=False)
        pc_queue = self.device.getOutputQueue(name="out", maxSize=1, blocking=False)
        ...
        if depth_message and pc_message:
            in_color = pc_message["rgb"]
            depth_frame = np.asarray(depth_message.getFrame())
            cv_color_frame = np.asarray(in_color.getCvFrame())

            color_raw = o3d.geometry.Image(cv_color_frame)
