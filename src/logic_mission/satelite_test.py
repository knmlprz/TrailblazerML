...
if __name__ == "__main__":
    ...
    communicator.send_message(SatelliteCommunicator.MSG_ID_NAVIGATE_GPS, gps_data)

    # Notify task completion
    communicator.task_completed()

    # Set a new stage (Example stage 1)
    communicator.send_message(SatelliteCommunicator.MSG_ID_SET_STAGE, b'\x01')

    # Locate Aruco Tags command
    communicator.send_message(SatelliteCommunicator.MSG_ID_LOCATE_ARUCO_TAGS, b'')

    # Send a detection command (assuming no additional body data is needed)
    communicator.send_message(SatelliteCommunicator.MSG_ID_DETECTION, b'')

    # Set parameters command (assuming no additional body data is needed)
    communicator.send_message(SatelliteCommunicator.MSG_ID_SET_PARAMETERS, b'')
