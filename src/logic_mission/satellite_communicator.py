...
class SatelliteCommunicator:
    ...
    def read_message(self):
        if self.serial_port.in_waiting > 0:
            ...
            if start_byte == struct.pack("B", self.START_BYTE):
                ...
                print(
                    f"l2 message_id {message_id} body_length {body_length} body {body.hex()} checksum {checksum}"
                )
                if self.verify_checksum(
                    start_byte
                    + struct.pack("B", message_id)
                    + struct.pack("B", body_length)
                    + body,
                    checksum,
                ):
                    ...
                    self.write_log(
                        "Message processed: ID {} Body Length {}".format(
                            message_id, body_length
                        )
                    )
                    print(
                        f"l3 Message processed: ID {message_id} Body Length {body_length}"
                    )
    ...
    def task_completed(self):
        self.send_message(
            self.MSG_ID_TASK_COMPLETED, b""
        )  # assuming no additional data needed
